...
palette_plot(moma.colors("Liu"))
palette_check(moma.colors("Liu"))
palette_dist(moma.colors("Liu"))
## Colorblind Checking 

CBTotal <- data.frame() 

for(pal_nam in names(Colors)){
CBCheck <- palette_check(moma.colors(pal_nam)) %>%
  as.data.frame %>%
...
palette_plot(moma.colors(pal_nam))
dev.off()
